...
class UciInput(private val inputHandler: IInputHandler) {
    ...
    fun process(command: String) {
        ...
        when (tokens[0]) {
            "uci" -> {
                //UciOutput.println("id name ChessV v" + PlatformSpecific.getVersion())
                UciOutput.println("id name ChessV v0")
                UciOutput.println("id author tdc")
                ...
                UciOutput.println("option name Threads type spin default 1 min 1 max 256")
